-- Recreate check_and_lock_slot with explicit locking and no lock extending if soft_lock_until > NOW()
...
BEGIN
    ...
    IF FOUND THEN
        ...
        ELSIF v_existing_sub.status = 'locked' AND v_existing_sub.soft_lock_until > NOW() THEN
            ...
            RETURN jsonb_build_object('success', true, 'status', 'locked', 'ticket_type', v_existing_sub.ticket_type, 'lock_expires_at', v_existing_sub.soft_lock_until, 'remaining_seconds', GREATEST(0, EXTRACT(EPOCH FROM (v_existing_sub.soft_lock_until - NOW()))), 'message', 'Bu TC ile hali hazırda devam eden bir işleminiz bulunmaktadır.');
        END IF;
    END IF;
...
-- Update get_table_stats to only fetch for confirmed submissions
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      COUNT(*)::int as count
    FROM public.cf_submissions
    WHERE status NOT IN ('rejected', 'cancelled', 'locked')
