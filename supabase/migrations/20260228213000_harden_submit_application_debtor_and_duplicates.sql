-- =============================================================================
-- Migration: Harden submit_application (debtor + duplicate protection)
-- Date: 2026-02-28
--
-- Fixes:
-- 1) submit_application now enforces whitelist and debtor checks in DB layer
--    (prevents direct RPC bypass of frontend checks)
-- 2) Duplicate submissions are hard-rejected for statuses:
--    pending / approved / asil / yedek
--    Re-submit is allowed only for locked / cancelled / expired
-- =============================================================================
...
BEGIN
  ...
  -- Whitelist check (DB-side security)
  ...
  -- Debt check (if is_debtor column exists)
  SELECT EXISTS (
  ...
  -- Existing submission check: hard reject duplicates for active/finalized statuses
  ...
  -- HARD quota: only confirmed/non-locked records, excluding this tc
  ...
  -- Insert/update (re-submit allowed only for locked/cancelled/expired rows)
