-- =============================================================================
-- Migration: Make table capacity admin-managed via cf_quota_settings
-- Date: 2026-03-03
-- =============================================================================
...
ADD COLUMN IF NOT EXISTS table_capacity integer NOT NULL DEFAULT 10;

UPDATE public.cf_quota_settings
SET table_capacity = 10
WHERE table_capacity IS NULL OR table_capacity < 1;
...
BEGIN
  ...
  SELECT COALESCE(table_capacity, 10)
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(table_capacity, 10) AS table_capacity
  INTO v_settings
  FROM public.cf_quota_settings
  ORDER BY updated_at DESC NULLS LAST
  LIMIT 1;

  v_table_capacity := COALESCE(v_settings.table_capacity, 10);
