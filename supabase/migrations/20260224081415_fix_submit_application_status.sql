...
BEGIN
  ...
  -- Get the default form ID (assuming there's only one main form)
  SELECT id INTO v_form_id FROM public.cf_forms LIMIT 1;
  
  IF v_form_id IS NULL THEN
    RAISE EXCEPTION 'Aktif bir başvuru formu bulunamadı (cf_forms tablosu boş).';
  END IF;

  -- Check if user attended before from whitelist
  ...
  -- Calculate total reserved (excluding current user's old tickets if editing)
  ...
  IF v_total_reserved + v_ticket_count > 1500 THEN
  ...
  IF v_attended_before THEN
    -- If attended before, they ALWAYS go to yedek
    v_ticket_type := 'yedek';
  ELSE
    IF v_total_reserved + v_ticket_count <= 700 THEN
      v_ticket_type := 'asil';
    ELSE
      v_ticket_type := 'yedek';
    END IF;
  ...
  v_result := json_build_object('success', true, 'ticket_type', v_ticket_type, 'attended_before', v_attended_before);
  RETURN v_result;
END;
$function$;
